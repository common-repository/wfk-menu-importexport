...
class Wfktyh_Mie_Ajax {
    ...
    public function func_select_menu_import_export() {
        check_ajax_referer( "security", 'select_importexport_nonce' );
    ...
    public function func_wfktyh_download_export_file() {
        check_ajax_referer( "security", 'menu_download_nonce' );
    ...
    public function func_wfktyh_import_file() {
        ...
        $menuFile = isset( $_FILES['menu_file'] ) ? $_FILES['menu_file'] : false;
        if ( $menuFile ) {
            $message = self::formFileValidate( $menuFile );
            ...
            if ( !\file_exists( $upload_path ) ) {
                if ( !\mkdir( $upload_path, 0777, true ) ) {
                    $message = __( "You don't have permission to make folder", "wfk-menu-importexport" );
                    wp_send_json_error( $message );
                }
            }
            ...
            $upload_overrides = ['test_form' => false, 'action' => 'wfktyh_import_file'];
            ...
            $movefile = \wp_handle_upload( $menuFile, $upload_overrides );
            ...
            if ( !$menuexists ) {
                ...
                if ( is_array( $file_content ) && !empty( $file_content ) ) {
                    ...
                    wp_send_json_error( __( "There was something wrong.", "wfk-menu-importexport" ) );
                }

            } else {
                wp_send_json_error( __( "Please select a unique menu, the given menu name exists.", "wfk-menu-importexport" ) );
            }
     ...
     */
    public static function formFileValidate( $menuFile ) {
        $message = '';
        if ( !\current_user_can( 'upload_files' ) ) {
            $message = __( "You don't have permission to upload a file.", "wfk-menu-importexport" );
        }
        $menuFileName = sanitize_file_name( $menuFile['name'] );
        if ( isset( $menuFileName ) ) {
            $wp_filetype = \wp_check_filetype( $menuFileName, ['json' => 'application/json'] );
            if ( !\wp_match_mime_types( 'application/json', $wp_filetype['type'] ) ) {
                $message = __( "Please upload your menu json file!", "wfk-menu-importexport" );
            }
        }
        if ( $menuFile['size'] > 2000000 ) {
            $message = __( "Your file size is too large!", "wfk-menu-importexport" );
        }
        return $message;
    }
    /*
